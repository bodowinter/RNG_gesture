...
```{r}
eng_2023_gender <- distinct(meta_eng_2023, id, gender)
eng_2024_gender <- distinct(meta_eng_2024, id, gender)
```
...
ita_2023 <- select(ita_2023,
                   id, gender, trial, movement, SNS, BAG, number)
...
pair_p <- df_ppt |>
  ggplot(aes(x = movement, y = M,
             group = id)) +
  geom_line(aes(col = effect), alpha = 0.8) +
  geom_point(aes(fill = movement), size = 2, alpha = 0.7,
             shape = 21, col = 'black')
...
diff_p <- as_draws_df(beta_mdl, variable = 'b_movementoutwards') |> 
  ...
  stat_halfeye(fill = 'purple', alpha = 0.8) +
  geom_vline(xintercept = 0, linetype = 'dashed')
...
diff_p <- diff_df |> 
  ...
  ggplot(aes(x = movement_towards,
             fill = `Relative change`, y = p)) +
...
**BANANA: Bodo: Maybe later make this into a publication-ready plot? Not a priority right now.**
